...
class ProviderBroker {
  constructor() {
    this.providers = [];
    this.providerInvalidations = new Map();
  ...
  addProviders(...providers) {
    for (const provider of providers) {
      ...
      const subscription = provider.onDidInvalidate?.((event) => {
        this.emitter.emit("did-invalidate-symbols", {
          editor: event?.editor ?? null,
          source: provider,
        });
      });
      if (subscription) this.providerInvalidations.set(provider, subscription);
    }
  }
  ...
  setSymbolRegistry(registry) {
    ...
    this.symbolRegistrySubscription = registry.onDidInvalidateFileSymbols((bundle) => {
      this.emitter.emit("did-invalidate-symbols", {
        ...bundle,
        source: this.symbolRegistryAdapter,
      });
    });
  }
  ...
  removeProvider(provider) {
    ...
    if (index > -1) {
      ...
      this.providerInvalidations.get(provider)?.dispose();
      this.providerInvalidations.delete(provider);
    }
  }
