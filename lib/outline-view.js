const { CompositeDisposable, Emitter, Range, TextEditor } = require("lumine");
...
class OutlineView {
  ...
  handleEvents() {
    this.disposables.add(
      ...
      // The symbol hub announces staleness itself — edits settling, saves,
      // grammar changes, providers coming and going. When its adapter is the
      // active provider, those events drive the refresh; a null editor in the
      // bundle means every editor is affected.
      this.broker.onDidInvalidateSymbols(({ editor }) => {
        if (!this.activeProvider?.refreshViaRegistry) return;
        if (!this.activeEditor) return;
      ...
      lumine.commands.add(this.element, {
        ...
        "outline-view:clear-search": () => this.clearSearch(),
        "outline-view:focus-search": () => this.focusSearch(),
        "outline-view:toggle-search-focus": () => this.toggleSearchFocus(),
  ...
  handleEditorEvents() {
    ...
    disposables.add(
      editor.onDidStopChanging(() => {
        // A registry-driven provider is refreshed by invalidation events
        // alone; refreshing here too would fetch twice per edit.
        if (this.activeProvider?.refreshViaRegistry) return;
  ...
  render() {
    ...
    if (symbolElements.length > 0) {
      ...
      contents = etch.dom(
        ...
        etch.dom("li", {}, this.searchResults ? "No results" : "No Symbols"),
      );
    }
