...
class OutlineView {
  ...
  handleEvents() {
    this.disposables.add(
      ...
      this.broker.onDidInvalidateSymbols(({ editor, source }) => {
        if (source && source !== this.activeProvider) return;
        if (!this.activeEditor) return;
  ...
  switchToEditor(editor) {
    ...
    this.activeProvider = this.broker.chooseProviderForEditor(editor);

    if (!this.activeProvider) {
      this.setSymbols(null);
    } else if (this.editorSymbolsList.has(editor)) {
